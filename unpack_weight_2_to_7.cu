...
int QbitGemv(SampleData* p_sampledata) {
  ...
  // DequantizeAndUnpackWeight248<half, 4><<<gridDim, blockDim>>>(b_fp16, qweight_i32, scale_fp16, qzeros_i32, 128, MATRIX_K / 8, MATRIX_N);
  DequantizeAndUnpackWeight3567<half, 4><<<gridDim, blockDim>>>(b_fp16, qweight_i32, scale_fp16, qzeros_i32, 128, MATRIX_K / 8, MATRIX_N);
  cudaErrCheck(cudaEventRecord(startGEMV));
  for (int i = 0; i < n_iter; i++) {
    DequantizeAndUnpackWeight248<half, 4><<<gridDim, blockDim>>>(b_fp16, qweight_i32, scale_fp16, qzeros_i32, 128, MATRIX_K / 8, MATRIX_N);
    //DequantizeAndUnpackWeight3567_v2<half, 4><<<gridDim, blockDim>>>(b_fp16, qweight_i32, scale_fp16, qzeros_i32, 128, MATRIX_K / 8, MATRIX_N);
  }
