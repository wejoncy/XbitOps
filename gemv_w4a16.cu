...
__global__ void gemv(T* out, T* inA, uint32_t* inB, T* scales, uint32_t* qzeros, int32_t groupsize) {
  ...
  half2 scale = ((half2*)(scales + start_group_id * MATRIX_N + n_offset_x))[0];
...
#pragma unroll
    ...
    if (g_id > start_group_id) {
      scale = ((half2*)(scales + g_id * MATRIX_N + n_offset_x))[0];
      qzero_p = ((int32_t*)(qzeros + n_offset_x / 8 + g_id * ((MATRIX_N + 7) / 8)))[0];
