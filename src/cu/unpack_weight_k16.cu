...
__global__ void DequantizeAndUnpackWeight(T* out, uint32_t* qweight, T* scale, uint32_t* zeros, int group_size, const int m, const int n) {
  ...
  int weight_in_row = tid / half_n;
  half2 scale_v = FETCH_HALF2(scale[weight_in_row * n + col_ind]);
  uint32_t zero_v = zeros[weight_in_row * (n / 8) + (col_ind) / 8];
  ...
  for (int kq = 0; kq < kNumQweights; kq++) {
    float2 weight_int2 = FETCH_FLOAT2(qweight[(weight_in_row*16+kq )* n + col_ind]);
...
#pragma unroll
    for (int i = 0; i < 32 / 4; i++) {
      ...
      out_h2[((wo+kq*8 + i) * n + col_ind) / 2] = __hfma2(wv, scale_v, -scale_zeros);
      // if (bid == 384 && threadIdx.x == 0) {
...
int QbitGemv(SampleData* p_sampledata) {
  ...
  // First: using GEMV
  dim3 gridDim = {(MATRIX_N * MATRIX_K / 8 + kBlockSize * 2 - 1) / kBlockSize / 2/kNumQweights};
  dim3 blockDim = {kBlockSize};

  printf("Running with DequantizeAndUnpackWeight...\n");
  DequantizeAndUnpackWeight<half><<<gridDim, blockDim>>>(b_fp16, qweight_i32, scale_fp16, qzeros_i32, 128, MATRIX_K / 8, MATRIX_N);
