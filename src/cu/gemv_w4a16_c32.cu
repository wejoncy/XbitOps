...
template <typename T>
__global__ void gemv(T* out, T* inA, uint32_t* inB, T* scales, uint32_t* qzeros, int32_t groupsize) {
  int bid = blockIdx.x;
  ...
  half2* inA_start = (half2*)(inA + blockIdx.y * MATRIX_K + y_start);
  ...
  float2 scale = __half22float2(((half2*)(scales + start_group_id * MATRIX_N + n_offset_x))[0]);
  int32_t qzero_p = ((int32_t*)(qzeros + n_offset_x / 8 +
                                start_group_id * ((MATRIX_N + 7) / 8)))[0];
  float2 hzero = {__int2float_rn((qzero_p >> (8 * (compressed_idx))) & 0xf),
...
#pragma unroll
  for (int i = 0; i < block_k / 2; i += 4) {  // read half2 * 4
    ...
    // if (k_offset / groupsize > start_group_id)
    {
      ...
      scale = __half22float2(((half2*)(scales + g_id * MATRIX_N + n_offset_x))[0]);
...
#pragma unroll
    for (int j = 0; j < 4; j++) {
      ...
      //                            0xF));
      vb[j] = {__int2float_rn(((*(qweight_p1 + j))) & 0xF),
               __int2float_rn(((*(qweight_p1 + j)) >> 4) & 0xF)};
      vb[j + 4] =
          {__int2float_rn(((*(qweight_p2 + j))) & 0xF),
           __int2float_rn((((*(qweight_p2 + j)) >> 4)) & 0xF)};
    }

    float2 va[4];
    va[0] = (k_offset < MATRIX_K) ? __half22float2(((inA_start))[i]) : res2;
    va[1] =
        (k_offset + 1 < MATRIX_K) ? __half22float2((inA_start)[i + 1]) : res2;
    va[2] =
        (k_offset + 2 < MATRIX_K) ? __half22float2((inA_start)[i + 2]) : res2;
    va[3] =
        (k_offset + 3 < MATRIX_K) ? __half22float2((inA_start)[i + 3]) : res2;
...
int QbitGemv(SampleData* p_sampledata) {
  ...
  cudaErrCheck(cudaMemcpy(a_fp16, sampledata.input, sampledata.size.input_size,
                          cudaMemcpyHostToDevice));
  cudaErrCheck(cudaMemcpy(qweight_i32, sampledata.qweight,
                          sampledata.size.qweight_size,
                          cudaMemcpyHostToDevice));
  cudaErrCheck(cudaMemcpy(scale_fp16, sampledata.scale,
                          sampledata.size.scale_size, cudaMemcpyHostToDevice));
  cudaErrCheck(cudaMemcpy(qzeros_i32, sampledata.qzero,
                          sampledata.size.qzero_size, cudaMemcpyHostToDevice));
  cudaErrCheck(cudaMemcpy(b_fp16, sampledata.f16weight,
                          sampledata.size.f16weight_size,
                          cudaMemcpyHostToDevice));
  cudaErrCheck(cudaMemcpy(out_fp16_ref, sampledata.out,
                          sampledata.size.out_size, cudaMemcpyHostToDevice));
  ...
  dim3 gridDim = {(MATRIX_N + width_element_per_block - 1) /
                  ...
                  uint32_t(sampledata.size.input_size / 4096 / 2)};
  dim3 blockDim = {32, (MATRIX_K + block_k - 1) / block_k};
  printf("%d================\n", (MATRIX_K + block_k - 1) / block_k);
  printf("Running with gemv...\n");
  gemv<half><<<gridDim, blockDim>>>(c_fp16, a_fp16, qweight_i32, scale_fp16, qzeros_i32, 128);
  ...
  convertFp16ToFp32<<<(MATRIX_M * MATRIX_N + 255) / 256, 256>>>(
      c_fp32, c_fp16, MATRIX_M * MATRIX_N);
  printf("............................\n\n");
  // Now using cuBLAS
  ...
  cudaErrCheck(cudaMemcpy(c_host_gemv, c_fp32,
                          MATRIX_M * MATRIX_N * sizeof(float),
                          cudaMemcpyDeviceToHost));
  ...
  for (int i = 0; i < MATRIX_M * MATRIX_N; i++) {
    ...
    if ((abs(ref / target) > 1.00001 || abs(target / ref) > 1.00001) ||
        abs(ref - target) > 3e-4) {
      errors++;
      if (errors < 10)
        printf("%d: r %f %f\n", i, ref, target);
